...
func NewMTProto(appId int32, appHash, authkeyfile, dcAddress string, debug int32) (*MTProto, error) {
	var err error
	m := new(MTProto)
	__debug = debug
	// if dcAddress == "" {
...
func (m *MTProto) send(r TL) (resp TL, err error) {
	respchan := make(chan TL, 1)
	m.queueSend <- packetToSend{r, respchan}
	resp = <-respchan

	if tlerr, ok := resp.(TL_rpc_error); ok {
		err = errors.New(tlerr.error_message)
	}
	return
}
